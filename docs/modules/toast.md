...
**`--ease-out` reuse instead of the originally-specced curve.** The source spec (`motion.jsx` prototype) called for `cubic-bezier(0.22, 1, 0.36, 1)` on toast entrance. This project defines exactly one easing curve, `--ease-out: cubic-bezier(0.16, 1, 0.3, 1)` (used by the only other animation in the codebase, `fade-slide-in`), under a stated project convention of no spring/bounce motion. Introducing a second, nearly-identical curve for one component would fragment that convention for no visible difference — toast entrance reuses `--ease-out`. Exit uses a literal `ease-out` (no reusable precedent existed for an exit transition).
